...
import {
  ...
  Gamepad2, CheckCircle, Send, Phone, User, Hash,
} from "lucide-react";
...
function RegisterModal({ tournament, onClose, onSubmitted }: { tournament: Tournament; onClose: () => void; onSubmitted: (tournamentId: string) => void }) {
  ...
  const allFilled = players.every((p) => p.gameId.trim() && p.nickname.trim()) && captainPhone.trim();
  ...
  return (
    ...
    >
      <motion.div
        ...
        className="w-full max-w-xl bg-[#0d0d14] border border-white/10 rounded-2xl overflow-hidden shadow-2xl max-h-[90vh] flex flex-col"
      >
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-5 border-b border-white/10 bg-gradient-to-r from-cyber-purple/20 to-transparent flex-shrink-0">
          <div>
        ...
        {done ? (
          <div className="px-6 py-12 text-center">
            ...
            <h3 className="font-display font-bold text-2xl text-white mb-2">Заявка отправлена!</h3>
            <p className="text-gray-400 text-sm mb-6">Администратор рассмотрит вашу заявку и свяжется с вами.</p>
            <button onClick={onClose} className="px-6 py-2.5 bg-gradient-to-r from-cyber-purple-bright to-cyber-neon text-white font-display font-bold rounded-xl hover:shadow-neon transition-all">
              Закрыть
            </button>
          </div>
        ) : (
          <div className="overflow-y-auto flex-1">
            <div className="p-6 space-y-6">
                ...
                <div className="grid grid-cols-3 gap-2">
                  {[5, 6, 7].map((n) => (
                    <button key={n} type="button" onClick={() => handleSizeChange(n)}
                      className={cn(
                        "py-3 rounded-xl font-display font-bold text-xl transition-all border",
                        teamSize === n
                          ? "bg-gradient-to-br from-cyber-purple-bright to-cyber-neon text-white border-transparent shadow-neon"
              ...
              <div>
                <label className="block text-xs font-mono text-gray-400 uppercase tracking-wider mb-3">
                  Состав команды
                  <span className="ml-2 text-yellow-400 normal-case">(Игрок 1 — капитан)</span>
                </label>
                      ...
                      <div className="flex-1 grid grid-cols-2 gap-2">
                        <div className="relative">
                          ...
                          <input
                            ...
                            className="w-full pl-8 pr-3 py-2 bg-white/5 border border-white/10 rounded-lg text-white text-sm placeholder-gray-600 focus:outline-none focus:border-cyber-neon/50 font-mono"
                          />
                        </div>
                        <div className="relative">
                          <User className="absolute left-3 top-1/2 -translate-y-1/2 w-3 h-3 text-gray-600" />
                          <input
                            value={p.nickname}
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-10 pr-4 py-2.5 bg-white/5 border border-white/10 rounded-xl text-white placeholder-gray-600 focus:outline-none focus:border-cyber-neon/50 font-mono text-sm"
                  />
                </div>
              </div>
              ...
              <div className="flex items-center gap-3 p-3 bg-white/5 border border-white/10 rounded-xl">
                <div className="w-9 h-9 rounded-full bg-gradient-to-br from-cyber-purple-bright to-cyber-neon flex items-center justify-center text-white font-bold text-sm flex-shrink-0">
                ...
                <span className="text-xs font-mono text-yellow-400 bg-yellow-500/10 border border-yellow-500/20 rounded px-2 py-0.5">Капитан</span>
              </div>
            </div>
          </div>
        )}
...
// Статус заявки текущего пользователя по ID турнира
...
export default function TournamentsPage() {
  ...
  useEffect(() => {
    if (!user) return;
    getDocs(query(collection(db, "tournament_applications"), where("captainId", "==", user.id)))
      .then((snap) => {
        const map: Record<string, AppStatus> = {};
        snap.docs.forEach((d) => { map[d.data().tournamentId] = d.data().status as AppStatus; });
        setMyApps(map);
      })
      .catch(() => {});
  }, [user]);
  ...
  return (
    <div className="min-h-screen pt-24 pb-16">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          {user?.role === "admin" && (
            <Link href="/admin/tournaments"
              ...
              <Zap className="w-4 h-4" />Организовать турнир
            </Link>
          )}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          {filtered.map((tournament) => (
            <TournamentCard key={tournament.id} tournament={tournament} onRegister={setRegistering} appStatus={myApps[tournament.id] ?? null} />
          ))}
        </div>
      ...
      <AnimatePresence>
        {registering && <RegisterModal tournament={registering} onClose={() => setRegistering(null)} onSubmitted={(tid) => setMyApps((prev) => ({ ...prev, [tid]: "pending" }))} />}
      </AnimatePresence>
    </div>
  );
}

function TournamentCard({ tournament, onRegister, appStatus }: {
    ...
    <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
      ...
      <div className="p-5 flex-1 flex flex-col">
        <Link href={`/tournaments/${tournament.id}`}>
          <h3 className="font-display font-bold text-base text-white hover:text-cyber-neon transition-colors leading-tight mb-1">{tournament.title}</h3>
        </Link>
        <p className="text-xs text-gray-500 font-mono mb-4">{tournament.game} • {tournament.format}</p>
        <div className="space-y-2.5 mb-4 flex-1">
          <div className="flex justify-between text-xs"><span className="flex items-center gap-1 text-gray-500"><DollarSign className="w-3.5 h-3.5" />Приз</span><span className="text-cyber-neon font-bold font-mono">{tournament.prizePool} UZS</span></div>
          <div className="flex justify-between text-xs"><span className="flex items-center gap-1 text-gray-500"><Calendar className="w-3.5 h-3.5" />Дата</span><span className="text-gray-300 font-mono">{tournament.startDate}</span></div>
          <div className="flex justify-between text-xs"><span className="flex items-center gap-1 text-gray-500"><Clock className="w-3.5 h-3.5" />Взнос</span><span className="text-gray-300 font-mono">{tournament.entryFee}</span></div>
        </div>
        <div className="mb-4">
          <div className="flex justify-between text-xs mb-1.5"><span className="flex items-center gap-1 text-gray-500"><Users className="w-3.5 h-3.5" />Команды</span><span className="text-gray-300 font-mono">{tournament.teamsRegistered}/{tournament.maxTeams}</span></div>
          <div className="h-1.5 bg-cyber-purple-mid rounded-full overflow-hidden">
            <div style={{ width: `${regPercent}%` }} className={cn("h-full rounded-full transition-all duration-700", regPercent >= 100 ? "bg-red-500" : "bg-gradient-to-r from-cyber-purple-bright to-cyber-neon")} />
          </div>
        </div>
        {canRegister && appStatus === "pending" ? (
          <div className="flex items-center justify-center gap-2 py-2.5 rounded-xl text-xs font-display font-semibold uppercase tracking-wider bg-yellow-500/10 border border-yellow-500/30 text-yellow-400 cursor-default">
            <Clock className="w-3.5 h-3.5" />На рассмотрении
          </div>
        ) : canRegister && appStatus === "approved" ? (
          <div className="flex items-center justify-center gap-2 py-2.5 rounded-xl text-xs font-display font-semibold uppercase tracking-wider bg-green-500/10 border border-green-500/30 text-green-400 cursor-default">
            <CheckCircle className="w-3.5 h-3.5" />Заявка принята
          </div>
        ) : canRegister ? (
          <button onClick={() => onRegister(tournament)}
            ...
            <Gamepad2 className="w-3.5 h-3.5" />Участвовать
          </button>
        ) : (
          <Link href={`/tournaments/${tournament.id}`}>
            <div className={cn("flex items-center justify-center gap-2 py-2.5 rounded-xl text-xs font-display font-semibold uppercase tracking-wider transition-all duration-300",
              tournament.status === "active" ? "bg-red-500/20 border border-red-500/40 text-red-400" : "bg-cyber-purple/20 border border-cyber-glass-border text-gray-400")}>
              {tournament.status === "active" ? "Смотреть Live" : tournament.status === "upcoming" ? "Подробнее" : "Архив"}
