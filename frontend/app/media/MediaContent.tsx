...
function VideoModal({ video, onClose }: { video: Video; onClose: () => void }) {
  ...
  const thumbnail =
    video.thumbnailUrl || (ytId ? `https://img.youtube.com/vi/${ytId}/maxresdefault.jpg` : "");
            ...
            <div className="flex items-start justify-between gap-4">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1.5">
                  <span className={cn("text-xs font-mono font-bold border rounded-full px-2.5 py-0.5", meta.color)}>
                    {meta.label}
                  </span>
            ...
            {CATS.map((c) => {
              ...
              return (
                <button
                  ...
                  onClick={() => setCat(c)}
                  className={cn(
                    "px-3 py-2 rounded-xl text-xs font-mono font-bold transition-all",
                    cat === c
                      ? "bg-cyber-neon/20 text-cyber-neon border border-cyber-neon/40"
                ...
                >
                  <div className={cn("relative h-64 lg:h-96 bg-gradient-to-br overflow-hidden", (catMeta[featured.category] ?? defMeta).gradient)}>
                    ...
                    <div className="absolute inset-0 flex items-center justify-center">
                      <motion.div
                        whileHover={{ scale: 1.1 }}
                        className="w-20 h-20 rounded-full bg-white/20 backdrop-blur-sm border border-white/30 flex items-center justify-center group-hover:bg-cyber-neon/30 group-hover:border-cyber-neon/60 transition-all"
                      >
                        <Play className="w-8 h-8 text-white fill-white ml-1" />
                      </motion.div>
                    </div>
                    ...
                    <div className="absolute top-4 right-4 bg-black/60 rounded-full px-3 py-1 text-white text-xs font-mono">
                      {featured.duration}
                    </div>
                    <div className="absolute bottom-4 left-4 right-4">
                      <h2 className="font-display font-bold text-xl text-white line-clamp-2 group-hover:text-cyber-neon transition-colors">
                        {featured.title}
                      </h2>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="p-3">
                      ...
                      <h3 className="text-sm font-semibold text-white mt-1 line-clamp-2 group-hover:text-cyber-neon transition-colors">
                        {video.title}
                      </h3>
